...
최근 회사의 시각화 프로젝트에서 여러 차트를 개발하였습니다. 그 중에서도 Rader 차트는 라이브러리 없이 SVG로 개발하였는데요!
이번 포스팅에서는 SVG로 Radar차트를 개발한 경험을 공유해보고자 합니다!

---

![Example Image](/asset/radar-chart.gif)

## Radar Chart

Radar Chart는 3개 이상의 데이터를 각 축별로 비교할 때 사용하면 좋은 그래프이다.

**Chart.js** 공식 문서: Radar Chart
...
비교하고 싶은 축의 개수(n)에 맞게 다각형이 결정된다. 예를들어, 비교하고 싶은 축이 3개라면 삼각형의 차트가, 6개라면 육각형의 차트가 생성된다.
...
```
<g/>태그는 svg 태그 요소들을 그룹하하는데 사용한다.
<g/>태그로 묶어 하위 요소들에게 스타일링을 일괄적으로 주는 게 가능하다.
공통적인 묶음이 되는 요소들은 `<g/>태그` 로 묶어주는게 좋다.
```
